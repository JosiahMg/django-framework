...
def save_model(obj, obj_type, obj_name):
    """
    对象保存
    :param obj:     对象
    :param obj_type:    对象类型，根据不同类型，保存在/models/obj_type下面
    :param obj_name:  对象名称
    :return:
    """
    obj_type_dir = os.path.join(project_dir, 'models', obj_type)
    make_dir_if_not_exists(obj_type_dir)
    joblib.dump(obj, os.path.join(obj_type_dir, obj_name + '.model'))
