...
该TD库操作脚本适合：
    1、TD库超级表的创建、复制
    2、基于超级表创建子表
    3、查询超级表或子表并返回[dict]格式数据
    4、查询超级表或子表并返回dataframe格式数据
    5、多线程批量插入数据
优点：
    1、操作简单，传参方便，调用即可
    2、涵盖TD的操作范围广
    3、批量插入时间快
    4、子表名称已定制：前缀+分割符+标签1+分割符+标签2......
    5、对于特殊子表名称可以自己设定，调用batch_insert函数即可
缺点：
    1、需要指定参数较多
    2、如果字段数较多，达到50个以上，可能调试max_rows, 确定合适值，通常不影响
"""
import os
...
class OperateTD:
    def __init__(self):
        try:
            self.client = RestClient(url=f"http://{TDengineConfig['host']}:{TDengineConfig['port']}",
                                     user=TDengineConfig['user'],
                                     password=TDengineConfig['password'],
                                     database=TDengineConfig['db_name'])
            logger.info(f"connect {TDengineConfig['host']}:{TDengineConfig['port']}, user: {TDengineConfig['user']}, "
                        f"database: {TDengineConfig['db_name']}")
    ...
    def batch_insert_by_tags(self, df, sup_table_name, tag_fields, child_tags, sub_prefix=None, sep='_', max_workers=8,
        ...
        :return:
        """
        # 需要将tags的顺序和数据库表中的字段名称顺序相同 以下实现排序功能
        tb_columns = self.query_table_cols(sup_table_name)
        order_tag_fields = []
        for col in tb_columns:
            if col in tag_fields:
                order_tag_fields.append(col)
        tag_fields = order_tag_fields
    ...
    def __df2td(self, df, sub_table_name, sup_table_name, tag_fields):
        ...
        if df.shape[0] > 0:
            try:
                tag_values = df[tag_fields][df[tag_fields].isnull().any(axis=1) == False].values.tolist()[0]
            except Exception as e:
                logger.error("字段标签值错误:{}".format(' Null values exist on all rows in the tag fields!'))
                raise ValueError(e)
            fields_df = df[list(set(df.columns).difference(set(tag_fields)))].copy()

            try:
                field_values_str = ""
                fields_df = fields_df.fillna(value='NULL')
